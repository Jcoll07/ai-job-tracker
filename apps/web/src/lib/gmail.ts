...
function extractBody(payload: GmailPart): string {
  ...
  if (payload.mimeType === "text/html" && payload.body?.data) {
    return decodeBase64Url(payload.body.data).replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/&nbsp;/g, " ").replace(/\s+/g, " ");
  }
  return "";
}
...
export async function syncGmail(): Promise<SyncResult> {
  ...
  for (const m of messages) {
    ...
    try {
      ...
      let statusApplied = 0;
      if (job && suggested && shouldAutoApply(job.status as JobStatus, suggested)) {
        const updated = updateJob(job.id, { status: suggested }, "gmail");
        if (updated) {
          statusApplied = 1;
          result.statusUpdates.push({ jobId: job.id, company: job.company, toStatus: suggested });
        }
      }
      db.prepare(`INSERT INTO emails (gmailId, threadId, jobId, fromAddress, subject, snippet, receivedAt, category, confidence, summary, suggestedStatus, statusApplied) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(m.id, m.threadId, job?.id ?? null, from, subject, full.snippet ?? "", receivedAt, classification.category, classification.confidence, classification.summary, suggested, statusApplied);
    } catch (err) {
      result.errors.push(`${m.id}: ${err instanceof Error ? err.message : "unknown error"}`);
    }
  }
