...
const TOKEN_URL="https://oauth2.googleapis.com/token",GMAIL_API="https://gmail.googleapis.com/gmail/v1/users/me",BATCH_SIZE=5,MAX_MESSAGES=10000;
...
async function accessToken(){const t=getSetting<any>("gmailTokens");if(!t)throw new Error("Gmail is not connected");if(Date.now()<t.expires_at-60000)return t.access_token;const c=creds();if(!c||!t.refresh_token)throw new Error("Gmail token expired and cannot be refreshed — reconnect Gmail");const r=await fetch(TOKEN_URL,{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({client_id:c.clientId,client_secret:c.clientSecret,refresh_token:t.refresh_token,grant_type:"refresh_token"})});if(!r.ok)throw new Error(`Token refresh failed: ${await r.text()}`);const d=await r.json() as any;const updated={...t,access_token:d.access_token,expires_at:Date.now()+d.expires_in*1000};setSetting("gmailTokens",updated);return updated.access_token}
async function gmailGet(path:string){const r=await fetch(`${GMAIL_API}${path}`,{headers:{authorization:`Bearer ${await accessToken()}`}});if(!r.ok)throw new Error(`Gmail API ${r.status}: ${await r.text()}`);return await r.json()}
function decode(data:string){return Buffer.from(data.replaceAll("-","+").replaceAll("_","/"),"base64").toString("utf8")}
function body(p:any):string{if(p.mimeType==="text/plain"&&p.body?.data)return decode(p.body.data);if(p.parts)for(const x of p.parts){const v=body(x);if(v)return v}if(p.body?.data)return decode(p.body.data);return ""}
function header(h:any[],n:string){return h.find(x=>x.name.toLowerCase()===n.toLowerCase())?.value??""}
function urls(text:string):string[]{return[...new Set((text.match(/https?:\/\/[^\s<>"']+/g)||[]).map(u=>u.replace(/[),.;]+$/g,"")))]}
function senderDomain(from:string){const at=from.lastIndexOf("@");return at>=0?from.slice(at+1).split(">")[0].trim():null}
function aboutTheJob(text:string):string|null{const normalized=text.replace(/\r/g,"");const stop="About the company|About the Company|Company|Benefits|Qualifications|Requirements|Skills|Responsibilities|What you(?:'|’)ll do|How you(?:'|’)ll make an impact|Employment type|Job function|Seniority level|Industry|Show less|Show more|Apply|Easy Apply|Report this job";for(const pattern of [new RegExp(`(?:^|\\n)\\s*About the Job\\s*[:\\-]?\\s*\\n([\\s\\S]*?)(?=\\n\\s*(?:${stop})\\b|$)`,`i`),new RegExp(`(?:^|\\n)\\s*About the Job\\s*[:\\-]?\\s*([\\s\\S]*?)(?=\\n\\s*(?:${stop})\\b|$)`,`i`)]){const value=normalized.match(pattern)?.[1]?.trim();if(value)return value}return null}
...
export async function syncGmailBatch(){
  ...
  for(const m of chunk){try{
    ...
    for(const parsed of postings){if(!parsed.company||!parsed.jobTitle)continue;let job=findDuplicate(parsed.company,parsed.jobTitle);if(!job){job=createJob({company:parsed.company,jobTitle:parsed.jobTitle,location:parsed.location,salaryRange:parsed.salaryRange,jobType:parsed.jobType,experience:parsed.experience,skills:parsed.skills,emailDomain:parsed.emailDomain??senderDomain(from),description:parsed.description||emailAbout||"",sourceUrl:parsed.sourceUrl||null,status:"Saved"},"gmail");result.created++}else{const updates:any={};if(parsed.description&&(job.description??"")!==parsed.description)updates.description=parsed.description;if(parsed.sourceUrl&&job.sourceUrl!==parsed.sourceUrl)updates.sourceUrl=parsed.sourceUrl;if(parsed.location&&job.location!==parsed.location)updates.location=parsed.location;if(parsed.salaryRange&&job.salaryRange!==parsed.salaryRange)updates.salaryRange=parsed.salaryRange;if(parsed.skills&&job.skills!==parsed.skills)updates.skills=parsed.skills;if(Object.keys(updates).length)job=updateJob(job.id,updates,"gmail")??job}if(job){if(!firstJobId)firstJobId=job.id;result.linked++}}
    if(!firstJobId&&classification){const linked=matchJobForEmail(senderDomain(from),classification.company==null?null:String(classification.company));if(linked){firstJobId=linked.id;result.linked++}}
    const suggested=(CATEGORY_TO_STATUS[classification?.category as keyof typeof CATEGORY_TO_STATUS] as JobStatus|undefined)??null;
