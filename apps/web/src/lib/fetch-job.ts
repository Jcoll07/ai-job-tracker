...
function isPrivateIp(address: string): boolean {
  ...
  if (version === 4) { const [a,b]=address.split(".").map(Number); return a===10||a===127||(a===169&&b===254)||(a===172&&b>=16&&b<=31)||(a===192&&b===168)||a===0; }
  if (version === 6) { const n=address.toLowerCase(); return n==="::1"||n==="::"||n.startsWith("fc")||n.startsWith("fd")||n.startsWith("fe8")||n.startsWith("fe9")||n.startsWith("fea")||n.startsWith("feb"); }
  return false;
}
async function assertSafeUrl(rawUrl:string):Promise<URL>{
  let url:URL; try{url=new URL(rawUrl)}catch{throw new Error("Invalid job posting URL")}
  ...
  try{const records=await lookup(hostname,{all:true,verbatim:true});if(!records.length||records.some(r=>isPrivateIp(r.address)))throw new Error("Private or loopback job posting URLs are not allowed")}catch(err){if(err instanceof Error&&err.message.includes("Private or loopback"))throw err;throw new Error("Could not resolve the job posting host")}
  return url;
}
async function fetchSafe(rawUrl:string):Promise<{url:string;html:string}>{
  ...
  for(let redirects=0;redirects<=MAX_REDIRECTS;redirects++){
    let res:Response;
    try{res=await fetch(current,{headers:{"user-agent":"Mozilla/5.0 (compatible; JobTrackr/2.0)",accept:"text/html,application/xhtml+xml,application/json;q=0.9,*/*;q=0.8"},redirect:"manual",signal:AbortSignal.timeout(FETCH_TIMEOUT_MS)})}
    catch(err){if(err instanceof Error&&(err.name==="TimeoutError"||err.name==="AbortError"))throw new Error("The job posting site took too long to respond (30s). If it requires JavaScript or blocks automated requests, use Copy & Paste instead.");throw new Error(`Unable to fetch the job posting: ${err instanceof Error?err.message:String(err)}`)}
    if(res.status>=300&&res.status<400){if(redirects===MAX_REDIRECTS)throw new Error("Too many redirects while fetching the job posting");const location=res.headers.get("location");if(!location)throw new Error("Job posting returned an invalid redirect");current=await assertSafeUrl(new URL(location,current).toString());continue}
    if(!res.ok)throw new Error(`Fetching the URL failed (HTTP ${res.status})`);
    const length=Number(res.headers.get("content-length")??0);if(length>MAX_RESPONSE_BYTES)throw new Error("Job posting response is too large (2 MB maximum)");
    const html=await res.text();if(Buffer.byteLength(html,"utf8")>MAX_RESPONSE_BYTES)throw new Error("Job posting response is too large (2 MB maximum)");return{url:current.toString(),html};
  }
  throw new Error("Unable to fetch job posting");
}
function htmlToText(html:string):string{return html.replace(/<script[\s\S]*?<\/script>/gi," ").replace(/<style[\s\S]*?<\/style>/gi," ").replace(/<\/(?:p|div|section|article|li|h[1-6]|br|tr)>/gi,"\n").replace(/<[^>]+>/g," ").replace(/&nbsp;|&amp;|&lt;|&gt;|&quot;|&#39;|&#x27;|&#\d+;/gi,m=>({"&nbsp;":" ","&amp;":"&","&lt;":"<","&gt;":">","&quot;":'"',"&#39;":"'","&#x27;":"'"}[m.toLowerCase()]??m)).replace(/[ \t]+/g," ").replace(/\n[ \t]+/g,"\n").replace(/[ \t]+\n/g,"\n").replace(/\n\s*\n+/g,"\n\n").trim()}
function clean(value:unknown):string|null{if(value===undefined||value===null)return null;const text=Array.isArray(value)?value.map(v=>clean(v)).filter(Boolean).join(", "):typeof value==="object"?JSON.stringify(value):String(value);const normalized=text.replace(/\s+/g," ").trim();return normalized||null}
...
export async function fetchJobPosting(url:string):Promise<{url:string;content:string;open:boolean;aboutTheJob:string|null;structuredJob:ScrapedJobPosting|null}>{
  ...
  for(const m of jsonLdBlocks){try{const data=JSON.parse(m[1]);const posting=parseJobPostingJsonLd(data,fetched.url,about);if(posting){const raw=JSON.stringify(data).slice(0,30000);return{url:fetched.url,content:`URL: ${fetched.url}\nJSON-LD JobPosting:\n${raw}${about?`\nABOUT_THE_JOB_VERBATIM:\n${about.slice(0,30000)}`:""}`,aboutTheJob:about,open:!looksClosed(raw),structuredJob:posting}}catch{}}
