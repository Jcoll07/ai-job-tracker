...
async function assertSafeUrl(rawUrl: string): Promise<URL> {
  ...
  if (hostname === "localhost" || hostname.endsWith(".localhost") || hostname.endsWith(".local") || hostname.endsWith(".internal")) throw new Error("Local/private job posting URLs are not allowed");
  if (isPrivateIp(hostname)) throw new Error("Private or loopback job posting URLs are not allowed");
  try {
    const records = await lookup(hostname, { all: true, verbatim: true });
    if (!records.length || records.some((r) => isPrivateIp(r.address))) throw new Error("Private or loopback job posting URLs are not allowed");
  } catch (err) {
  ...
  for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
    ...
    try {
      ...
      if (err instanceof Error && (err.name === "TimeoutError" || err.name === "AbortError")) {
        throw new Error("The job posting site took too long to respond (30s). If it requires JavaScript or blocks automated requests, use Copy & Paste instead.");
      }
      throw new Error(`Unable to fetch the job posting: ${err instanceof Error ? err.message : String(err)}`);
    }
...
function extractAboutTheJob(text: string): string | null {
  ...
  const stop = "About the company|About the Company|Company|Benefits|Qualifications|Requirements|Skills|Responsibilities|What you(?:'|’)ll do|How you(?:'|’)ll make an impact|Employment type|Job function|Seniority level|Industry|Show less|Show more|Apply|Easy Apply|Report this job";
  const patterns = [new RegExp(`(?:^|\\n)\\s*About the Job\\s*[:\\-]?\\s*\\n([\\s\\S]*?)(?=\\n\\s*(?:${stop})\\b|$)`, "i"), new RegExp(`(?:^|\\n)\\s*About the Job\\s*[:\\-]?\\s*([\\s\\S]*?)(?=\\n\\s*(?:${stop})\\b|$)`, "i")];
  for (const pattern of patterns) { const value = normalized.match(pattern)?.[1]?.trim(); if (value) return value; }
  return null;
}
...
export async function fetchJobPosting(url: string): Promise<{ url: string; content: string; open: boolean; aboutTheJob: string | null }> {
  ...
  for (const m of jsonLdBlocks) {
    try {
      ...
      const posting = items.find((i: any) => i?.["@type"] === "JobPosting");
      if (posting) {
        const status = String(posting.validThrough ?? "");
        const closing = String(posting.jobStatus ?? "");
        const closed = looksClosed(JSON.stringify(posting)) || /closed|filled|expired/i.test(closing) || (status && Date.parse(status) < Date.now());
        const structured = `URL: ${fetched.url}\nJSON-LD JobPosting:\n${JSON.stringify(posting).slice(0, 30000)}`;
        return { url: fetched.url, content: about ? `${structured}\nABOUT_THE_JOB_VERBATIM:\n${about.slice(0, 30000)}` : structured, aboutTheJob: about, open: !closed };
      }
    } catch {}
  }
