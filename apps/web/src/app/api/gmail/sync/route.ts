...
async function runBatch() {
  ...
  try { return NextResponse.json({ result: await syncGmailBatch() }); }
  catch (err) { return NextResponse.json({ error: err instanceof Error ? err.message : "Sync failed" }, { status: 502 }); }
}
...
export async function GET(req: NextRequest) {
  ...
  if (!secret || req.headers.get("authorization") !== `Bearer ${secret}`) {
    if (isLocalRequest(req)) return NextResponse.json({ state: getGmailSyncState() });
