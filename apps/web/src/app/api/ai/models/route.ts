...
export async function GET(req: NextRequest) {
  ...
  const configured = [process.env.AI_MODEL, process.env.AI_MODEL_PARSE, process.env.AI_MODEL_CLASSIFY, process.env.AI_MODEL_CONTEXT, process.env.AI_MODEL_CV, process.env.AI_MODEL_ANSWERS].filter(Boolean) as string[];
  ...
  try {
    ...
    return NextResponse.json({ models: [...new Set([...models, ...configured, ...stored])], source: "local" });
  } catch (error) {
    return NextResponse.json({ models: [...new Set([...configured, ...stored])], source: "configured", warning: error instanceof Error ? error.message : "Unable to reach local AI server" });
  }
}
