...
EXT_DIR="$ROOT_DIR/apps/extension"; OUTPUT_DIR="$EXT_DIR/.output/safari-mv2"; PROJECT_DIR="$EXT_DIR/.output/safari-xcode"; INSTALL_DIR="${HOME}/Applications"; CONTAINER_APP="$INSTALL_DIR/JobTrackr Safari Extension.app"; DERIVED_DIR="$EXT_DIR/.output/safari-derived"; MACOS_DEPLOYMENT_TARGET="26.0"
...
# The Safari packager can generate an older deployment target. This project is macOS-only,
# so force both the containing app and Safari extension targets to the minimum supported macOS.
python3 - "$PBXPROJ" "$MACOS_DEPLOYMENT_TARGET" <<'PY'
...
updated, count = re.subn(r"MACOSX_DEPLOYMENT_TARGET = [^;]+;", f"MACOSX_DEPLOYMENT_TARGET = {target};", text)
if count == 0:
