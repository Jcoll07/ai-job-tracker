...
EXT_DIR="$ROOT_DIR/apps/extension"; OUTPUT_DIR="$EXT_DIR/.output/safari-mv2"; PROJECT_DIR="$EXT_DIR/.output/safari-xcode"; INSTALL_DIR="${HOME}/Applications"; CONTAINER_APP="$INSTALL_DIR/JobTrackr Safari Extension.app"; DERIVED_DIR="$EXT_DIR/.output/safari-derived"; MACOS_DEPLOYMENT_TARGET="26.0"; APP_BUNDLE_ID="com.jcoll07.jobtrackr"; EXTENSION_BUNDLE_ID="${APP_BUNDLE_ID}.Extension"
...
xcrun "$PACKAGER" "$OUTPUT_DIR" --project-location "$PROJECT_DIR" --app-name "JobTrackr Safari Extension" --bundle-identifier "$APP_BUNDLE_ID" --macos-only --copy-resources --no-open --no-prompt --force
...
# The generated project must use a parent/child Bundle ID hierarchy: the Safari extension
# identifier must be prefixed by the containing macOS application's Bundle ID.
python3 - "$PBXPROJ" "$MACOS_DEPLOYMENT_TARGET" "$APP_BUNDLE_ID" "$EXTENSION_BUNDLE_ID" <<'PY'
...
text, deployment_count = re.subn(r"MACOSX_DEPLOYMENT_TARGET = [^;]+;", f"MACOSX_DEPLOYMENT_TARGET = {target};", text)
if deployment_count == 0:
    raise SystemExit("Generated Safari Xcode project does not define MACOSX_DEPLOYMENT_TARGET.")
text, bundle_count = re.subn(r"PRODUCT_BUNDLE_IDENTIFIER = [^;]+;", f"PRODUCT_BUNDLE_IDENTIFIER = {app_id};", text, count=1)
if bundle_count != 1:
    raise SystemExit("Could not identify the containing app PRODUCT_BUNDLE_IDENTIFIER in the generated project.")
# The extension target is generated after the app target. Replace its bundle identifier
# separately so it remains a child of the app identifier.
marker = "PRODUCT_BUNDLE_IDENTIFIER = " + app_id + ";"
idx = text.find(marker)
if idx < 0:
    raise SystemExit("Containing app Bundle ID was not found after project normalization.")
next_idx = text.find("PRODUCT_BUNDLE_IDENTIFIER = ", idx + len(marker))
if next_idx < 0:
    raise SystemExit("Could not identify the Safari extension PRODUCT_BUNDLE_IDENTIFIER.")
...
if [ "${CI:-}" != "true" ]; then
  ...
  EXT_BUNDLE_ID=$(plutil -extract CFBundleIdentifier raw -o - "$APPEX/Contents/Info.plist" 2>/dev/null || true); [ -n "$EXT_BUNDLE_ID" ] || { printf '%s\n' "Could not read the embedded Safari extension bundle identifier." >&2; exit 1; }; [ "$EXT_BUNDLE_ID" = "$EXTENSION_BUNDLE_ID" ] || { printf '%s\n' "Unexpected embedded extension Bundle ID: $EXT_BUNDLE_ID (expected $EXTENSION_BUNDLE_ID)." >&2; exit 1; }
  APP_INFO_BUNDLE_ID=$(plutil -extract CFBundleIdentifier raw -o - "$CONTAINER_APP/Contents/Info.plist" 2>/dev/null || true); [ "$APP_INFO_BUNDLE_ID" = "$APP_BUNDLE_ID" ] || { printf '%s\n' "Unexpected containing app Bundle ID: $APP_INFO_BUNDLE_ID (expected $APP_BUNDLE_ID)." >&2; exit 1; }
  open "$PROJECT"; open "$CONTAINER_APP"; open -a Safari >/dev/null 2>&1 || true
  if command -v pluginkit >/dev/null 2>&1; then
    ...
    if [ "$REGISTERED" -ne 1 ]; then printf '%s\n' "WARNING: Safari has not registered the extension yet. Safari requires the containing app to be opened and, for unsigned development builds, Allow Unsigned Extensions." >&2; fi
  fi
  printf '\nJobTrackr Safari extension built and installed.\nXcode project: %s\nApp: %s\nExtension: %s\nApp Bundle ID: %s\nExtension Bundle ID: %s\nDeployment target: macOS %s\n' "$PROJECT" "$CONTAINER_APP" "$APPEX" "$APP_BUNDLE_ID" "$EXTENSION_BUNDLE_ID" "$MACOS_DEPLOYMENT_TARGET"
else printf '\nSafari Xcode project and containing app validated in CI:\nProject: %s\nApp: %s\nApp Bundle ID: %s\nExtension Bundle ID: %s\nDeployment target: macOS %s\n' "$PROJECT" "$CONTAINER_SOURCE" "$APP_BUNDLE_ID" "$EXTENSION_BUNDLE_ID" "$MACOS_DEPLOYMENT_TARGET"; fi
